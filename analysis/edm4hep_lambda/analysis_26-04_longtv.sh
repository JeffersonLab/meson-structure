...
BASE_CAMPAIGN=/work/eic3/users/romanov/meson-structure-2026-04-check/dd4hep_26-04_longtv
OUTPUT_DIR=/work/eic3/users/romanov/meson-structure-2026-04-check/ana_26-04_longtv
...
for energy in "${energies[@]}"; do
    ...
    singularity exec \
        ...
        -B "$OUTPUT_DIR":/output \
        "$IMG" \
        bash -c "cd /code && root -x -l -b -q 'mcpart_lambda.cxx(\"/work/*.edm4hep.root\", \"/output/${energy}_lambda\", 50000)'"
    ...
    singularity exec \
        ...
        -B "$OUTPUT_DIR":/output \
        "$IMG" \
        bash -c "cd /code && root -x -l -b -q 'acceptance.cxx(\"/work/*.edm4hep.root\", \"/output/${energy}_acceptance\", 50000)'"
