...
singularity exec \
    ...
    -B "$OUTPUT_DIR":/output \
    "$IMG" \
    bash -c 'cd /code && root -x -l -b -q '"'"'mcpart_lambda.cxx("/work/*.edm4hep.root", "/output/ana_lam_10x100_26-04", 5000)'"'"''
...
singularity exec \
    ...
    -B "$OUTPUT_DIR":/output \
    "$IMG" \
    bash -c 'cd /code && root -x -l -b -q '"'"'acceptance.cxx("/work/*.edm4hep.root", "/output/ana_acc_10x100_26-04", 5000)'"'"''
