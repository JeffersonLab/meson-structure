#!/usr/bin/env python3
"""
prot_acceptance_plots.py

Usage:
    python prot_acceptance_plots.py acceptance_file hits_file -o results
"""
import argparse
...
def plot_and_tabulate(path_acceptance: Path, path_hits: Path, outdir: Path):
    ...
    RP_hits = "prot_ForwardRomanPotHits"
    LFHCAL_hits = "prot_LFHCALHits"
    B0_tracker_hits = "prot_B0TrackerHits"
    ...
    stats_path = outdir / "prot_acceptance_stats.csv"
    ...
    make_and_save_hist2d(b0_df['x'], b0_df['y'], bins=(100,100),
                        out_path=outdir / "prot_B0.png",
                        title=r"1st Layer B0 Tracker Acceptance $p$",
                        xlabel="x (mm)", ylabel="y (mm)")

    make_and_save_hist2d(rp_df['x'], rp_df['y'], bins=(100,100),
                        out_path=outdir / "prot_RP.png",
                        title=r"1st Layer Roman Pots Acceptance $p$",
                        xlabel="x (mm)", ylabel="y (mm)")

    make_and_save_hist2d(lfhcal_df['x'], lfhcal_df['y'], bins=(100,100),
                        out_path=outdir / "prot_LFHCAL.png",
                        title=r"LFHCAL Acceptance $p$",
                        xlabel="x (mm)", ylabel="y (mm)")

    return {
        "stats_csv": str(stats_path),
        "b0_png": str(outdir / "prot_B0.png"),
        "rp_png": str(outdir / "prot_RP.png"),
        "lfhcal_png": str(outdir / "prot_LFHCAL.png"),
    }
