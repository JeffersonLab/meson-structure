...
#   glob     - glob pattern to find inputs
# ---------------------------------------------------------------------------
ANALYSES = [
    {
        'name': 'edm4hep_mcpart_lambda',
        'script': 'analysis/edm4hep_lambda/mcpart_lambda.cxx',
        'glob': '*.edm4hep.root',
    },
    {
        'name': 'edm4hep_acceptance',
        'script': 'analysis/edm4hep_lambda/acceptance.cxx',
        'glob': '*.edm4hep.root',
    },
]
...
def generate_container_script(analysis_name, energy, script_path, output_dir, input_files):
    ...
    /usr/bin/time -v root -x -l -b -q 'mcpart_lambda.cxx("{input_files.join(",")}","{output_dir}",5000)' 2>&1
